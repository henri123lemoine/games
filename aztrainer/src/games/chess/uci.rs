...
//! `UCI_LimitStrength`/`UCI_Elo`). One engine process per game; positions are
//! sent as FEN, so the client stays stateless per move. Reads go through a
//! forwarding thread so a hung engine times out instead of wedging an
...
impl Uci {
    /// Spawns `path` and configures it to play at `elo` (clamped by the engine
    /// to its supported range; Stockfish 17: 1320–3190).
