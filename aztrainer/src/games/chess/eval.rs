...
//! opponents — uniform random, `AlphaBeta(MaterialEval)` at depths 1–3, diluted
//! material, and Stockfish at a calibrated `UCI_Elo` — with paired random
//! openings and draws scored ½. All opponents' games run in one pool so the GPU
//! sees wide batches. The net drives `solvers::azero::Search<Chess>` directly.
...
pub enum Opponent {
    Random,
    AbMaterial(u32),
    /// Depth-1 material search playing a uniform-random move `pct_random`% of
    /// the time — fills the rating gap between `Random` and `AbMaterial(1)`.
    Diluted {
        pct_random: u32,
    },
                ...
                loop {
                    ...
                    match gather {
                        ...
                        Gather::Done => {
                            ...
                            let m = g.search.root_actions()[choice];
                            g.apply(m);
                            // No tree reuse here: the opponent moves before our
...
/// calibrating the ladder's Elo anchors. Returns `a`'s score and the game count.
