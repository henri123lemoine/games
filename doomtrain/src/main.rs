...
fn ppo_trace_cmd(iwad: &str, arena: &str, device: Device, steps: usize) {
    ...
    for _ in 0..steps {
        ...
        let mut sf: Vec<f32> = vec![
            ...
            s.frags as f32,
            s.deaths as f32,
            s.opponent_visible as f32,
            s.opp_bearing_deg,
            s.opp_dist,
            s.opp_rel_vx,
            s.opp_rel_vy,
            s.opp_health as f32,
            s.opp_memory.valid as f32,
            s.opp_memory.ticks_since_seen as f32,
            s.opp_memory.last_bearing_deg,
            s.opp_memory.last_dist,
        ];
