...
/// current net vs an older snapshot of itself at low sims. A win rate above
/// 0.5 means the net is still improving. Appends `{"event":"rate",...}`;
/// `--watch N` repeats every N minutes.
...
/// fixed baseline ladder (random / greedy / shallow MCTS). There is no
/// external engine anchor for 1v1 snake, so this is a plain win-rate panel
/// rather than a calibrated Elo. `--watch N` re-runs every N minutes.
