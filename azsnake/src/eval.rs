...
//! fixed opponents — uniform random, a one-ply greedy [`DuelEval`] agent, and
//! rollout MCTS over [`DuelEval`] — with paired random openings. All net
...
impl EvalGame {
    /// Resolves chance, plays opponent plies, and checks termination;
    /// afterwards the game is either finished or it is the net's turn.
...
/// (net A wins, total games). The KataGo-style relative progress signal:
/// A = current net, B = an older snapshot, so a win rate above 0.5 means the
/// net is still improving.
...
/// A random opening: resolve the initial food spawn, then play
/// `OPENING_PLIES` uniform random plies, resolving any food spawn between
/// them, so paired games diverge from a shared but varied start.
