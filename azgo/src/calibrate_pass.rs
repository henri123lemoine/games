...
//! training — a calibration check. `azgo calibrate-pass [--net ...] [--games 4]
//! [--sims 8]`.
...
fn own_abs(model: &Model, game: &Go, enc: &GoEncoder, s: &GoState) -> Vec<f32> {
    let mover = model.ownership_at(&enc.encode_state(game, s), SIZE).unwrap();
    let sign = if s.to_move() == 0 { 1.0 } else { -1.0 };
    mover.iter().map(|o| o * sign).collect()
}

fn best_move(model: &Model, game: &Go, enc: &GoEncoder, s: &GoState, sims: u32, noise: f32, rng: &mut Rng) -> GoAction {
    let cfg = PuctConfig {
        sims,
        max_leaves: 8,
        root_noise: noise,
    ...
    while let Gather::Requests(reqs) =
        search.advance(game, enc, s, &cfg, rng, std::mem::take(&mut results), &|_| false)
    {
...
pub fn run(args: &[String]) {
    let net_path: String = arg(args, "--net", "web/app/public/azero/azero-go.azweb".to_string());
    ...
    println!("calibrate-pass: ownership adjudication vs GNU Go final_score ({games} games, {sims} sims)");
    ...
    for g in 0..games {
        ...
        while !game.is_terminal(&s) && ply < 220 {
            ...
            gtp.cmd(&format!("play {color} {}", label.to_uppercase())).expect("gnugo play");
            game.apply(&mut s, a);
            ply += 1;
            if !decided {
                ...
                if game.result_decided(&oa, TAU) {
                    ...
                    match gnugo {
                        Some(gs) => {
                            ...
                            println!("  gnugo {gs:+.1}  winner {}", if agree { "AGREE" } else { "DIFFER" });
                        }
        ...
        println!(
            ...
            gnugo.map(|x| format!("  gnugo {x:+.1}")).unwrap_or_default()
        );
    }
    if total > 0 {
        println!(
