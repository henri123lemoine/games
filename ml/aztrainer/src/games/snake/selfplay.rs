...
//! chance nodes encountered during descent, but `Search::advance` requires the
//! *root* to be a player node — so each game resolves any pending food spawn
//! before searching its position.
...
/// Advances `state` through any pending food-spawn chance nodes until a player
/// is on the clock or the game is terminal.
...
pub struct SelfPlayConfig {
    ...
    /// value, so a fast win still outscores a slow one.
    pub gamma: f32,
...
/// Length difference at which the margin term saturates: a `tanh` over the raw
/// cell difference, so a few cells of lead already register but the term stays
/// bounded.
const MARGIN_SCALE: f32 = 8.0;

/// Seat 0's terminal value in `[-1, 1]`, sharpened by the length margin:
/// decisive games push toward the winner with a margin-scaled magnitude, and a
/// cap draw leans toward the longer snake instead of flat 0.
...
impl Worker {
    ...
    fn roll_move(&mut self, cfg: &SelfPlayConfig) {
        ...
        // A pool opponent's reply is off-policy for the current net: play it out
        // (so the hero faces a real opponent) but never record it as a target.
    ...
    fn play_move(&mut self, cfg: &SelfPlayConfig) -> Option<WorkerStep> {
        ...
        self.plies += 1;
        // Tree reuse extracts the subtree under the played edge. The search bakes
        // a *sampled* food cell into the tree at expansion; resolving the live
        // game's chance node draws an independent cell, so reuse is only sound
        // when no meal happened (the successor is a player node).
    ...
    fn finish(&mut self, cfg: &SelfPlayConfig, z_seat0: f32, end: GameEnd) -> WorkerStep {
        ...
        let samples = self
            ...
            .collect();
        // Resignation calibration and false-positive accounting key off the sign
        // of the outcome, not its magnitude.
...
pub struct SelfPlay {
    ...
    /// Past-checkpoint nets a fraction of games play as seat 1. Refreshed by the
    /// trainer via [`SelfPlay::set_pool`]; empty means pure self-play.
    pool: Vec<Infer>,
}
...
impl SelfPlay {
    ...
    /// Replaces the opponent pool with the given past-checkpoint nets. New games
    /// (after each finishes) draw from the updated pool; in-flight games keep the
    /// net they started with.
