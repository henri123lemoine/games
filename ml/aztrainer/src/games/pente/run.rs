//! The pente training entry: the batched self-play + tch training loop, the
//! benchmark, and helpers shared with the gauge. The net/optimizer/replay/
//! run-dir machinery is the shared `aztrainer` core; this module supplies
//! pente's config (global-pool spatial head, no auxiliary heads), the
//! self-play pool, and the per-iteration metric fields. Pente is a simplified
//! two-player board game: no komi, no pass, no ownership/score targets.
...
const DASHBOARD: &str = include_str!("../../../../../assets/azpente_dashboard.html");

pub fn config(blocks: usize, channels: i64, size: i64) -> NetConfig {
    NetConfig {
        blocks,
        channels,
        planes: pente::encode::PLANES as i64,
        size,
...
pub fn run(args: &[String]) {
    ...
    let sims: u32 = arg(args, "--sims", 256);
    ...
    let full_sims: u32 = arg(args, "--full-sims", sims.max(400));
    ...
    let mut pool = SelfPlay::new(sp_cfg, size as usize, mix(0x60A1_5EED, 0));
    ...
    loop {
        ...
        println!(
            "iter {iter:>4} [{elapsed_min:>6.1}m] loss {:.3} (p {policy_loss:.3} + v {value_loss:.3}) | \
             {} games, {} black wins ({} resign, {} draw), avg {:>3.0} plies, buffer {:>7} | \
...
pub fn bench(args: &[String]) {
    ...
    let game = Pente::new(size as usize);
    let enc = PenteEncoder::new(size as usize);
    let synth = |n: usize| {
        let mut s = game.initial_state();
        // Past the forced-center opening so the support is a typical width.
        let a0 = game.legal_actions(&s);
        game.apply(&mut s, a0[0]);
