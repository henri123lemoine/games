//! Interactive frontends for a checkpoint: terminal play, and the minimal UCI
//! server that chess GUIs and the local web board speak to. Both drive
//! `solvers::azero::Search<Chess>` directly through a small synchronous
//! advance/evaluate loop ([`run_to_done`]).

use std::collections::HashMap;
use std::path::PathBuf;

use chess::encode::PlanesEncoder;
use chess::{Board, Chess};
use game_core::Rng;
...
use crate::net::Infer;
use crate::rundir::epoch_secs;

/// Drives `search` to its visit budget against a synchronous evaluator — the
/// advance/evaluate loop every non-batched caller (play, uci) writes. `keys`
/// answers "did this position already occur?" for the cycle-draw guard.
fn run_to_done(
    search: &mut azero::Search<Chess>,
    board: &Board,
    keys: &HashMap<u64, u8>,
    cfg: &PuctConfig,
    rng: &mut Rng,
    mut eval: impl FnMut(&[crate::net::EvalRequest]) -> Vec<crate::net::EvalResult>,
) {
...
/// Play against a checkpoint from the terminal: moves in coordinate notation
/// (e2e4, e7e8q), `quit` to leave.
pub fn play(args: &[String]) {
    ...
    loop {
        ...
        let m = if human_turn {
            ...
            run_to_done(&mut search, &board, &keys, &puct, &mut rng, |reqs| {
                infer.forward_batch(reqs)
            });
            let i = argmax(search.root_visits());
            let m = search.root_actions()[i];
...
/// Minimal UCI engine over stdin/stdout: enough for chess GUIs and the local web
/// board (position startpos|fen [moves ...], go [movetime N]).
pub fn uci_engine(args: &[String]) {
    ...
    let cfg = net_config_for(args, &net_path);
    let infer = Infer::load(&net_path, cfg, device_for(), Kind::Half).unwrap_or_else(|e| {
    ...
    for line in stdin.lock().lines() {
        ...
        match words.next() {
            Some("uci") => {
                println!("id name azero-aztrainer ({}x{})", cfg.blocks, cfg.channels);
            ...
            Some("position") => {
                ...
                if rest.get(move_idx) == Some(&"moves") {
                    for text in &rest[move_idx + 1..] {
                        // A bad move here means engine and GUI disagree on the
                        // position — bail loudly rather than desync.
                        let legal = text
                            .parse::<chess::Move>()
                            .ok()
            ...
            Some("go") => {
                ...
                println!("bestmove {}", search.root_actions()[i]);
            }
            Some("quit") => break,
            _ => {}
        }
