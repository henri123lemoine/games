...
//! single batch (the CPU side — movegen, tree walking, encoding — runs
//! rayon-parallel over the games). Games persist across `collect` calls, so an
//! iteration boundary never abandons work.
//!
//! Chess drives `solvers::azero::Search<Chess>` directly (no per-game wrapper):
//! the search runs with `cycle_draws` on and a `seen` closure over the game's
//! repetition history, so a position that already occurred backs up a draw —
//! the threefold awareness the tree would otherwise lack.

use std::collections::HashMap;

use chess::encode::PlanesEncoder;
use chess::encode::az_move_index;
use chess::{Adjudication, Board, Chess, adjudicate};
use game_core::Rng;
use game_core::rand::sample_visits;
use rayon::prelude::*;
use solvers::azero::{self, Gather, PuctConfig, argmax};

use super::sample::{Sample, compact_planes};
...
pub struct SelfPlayStats {
    ...
    pub capped: u32,
    pub plies: u64,
    /// Resign-disabled games where the would-resign side did NOT lose: direct
    /// measure of resignation false positives.
...
impl Worker {
    ...
    fn advance(&mut self, cfg: &SelfPlayConfig, mut results: Vec<EvalResult>) -> WorkerStep {
        loop {
            // Disjoint field borrows so the `seen` closure's immutable borrow of
            // `key_counts` does not clash with the mutable borrows of `search`
            // and `rng` in the same call.
            let Worker {
                search,
                board,
                rng,
    ...
    fn play_move(&mut self, cfg: &SelfPlayConfig) -> Option<WorkerStep> {
        ...
        let moves = self.search.root_actions().to_vec();
    ...
    fn finish(&mut self, z_white: f32, end: GameEnd) -> WorkerStep {
        ...
        // Non-losing sides' minimum Q from control games: the distribution the
        // resignation threshold is calibrated against.
...
impl SelfPlay {
    ...
    /// finished games. Unfinished games stay parked (with their pending leaf
    /// results delivered) for the next call. Returns samples, stats, and the
    /// resignation-calibration pool.
    pub fn collect(
        &mut self,
        infer: &Infer,
            ...
            let outcomes: Vec<(Option<Finished>, Vec<EvalRequest>)> = self
                ...
                .map(|(w, r)| match w.advance(&cfg, std::mem::take(r)) {
                    ...
                    WorkerStep::Finished(s, plies, z, end, fp, calib) => {
                        // Deal the next game immediately so the batch keeps its
                        // width; a fresh game always needs a root eval.
                        w.reset(&cfg);
