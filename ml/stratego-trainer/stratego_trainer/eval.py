...
def play_matches(hero_move, hero_setup, opp_move, opp_setup, hero_seat,
    ...
    `opp_move`/`opp_setup` may be None -> opponent plays uniform-random.
    `hero_temperature` sharpens only the hero's sampling (the opponent net, if
    any, plays at temperature 1.0). Returns (hero_wins, hero_draws, hero_losses).
    """
...
def win_share(hero_move, hero_setup, opp_move, opp_setup, num_envs, games,
              move_cap, seed, hero_temperature=1.0):
